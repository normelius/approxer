...
callback_funcs = {
  "ode_first_order": ctypes.CFUNCTYPE(None, ctypes.c_double, ctypes.POINTER(ctypes.c_double), 
    ctypes.c_ulong, ctypes.POINTER(ctypes.c_double))
}
...
def euler(f: Callable, y: npt.NDArray[np.float64] = np.array([1.], dtype = np.float64), 
          iterations: int = 1000, t_0: float = 0.0, t: float = 10.0):
  """
